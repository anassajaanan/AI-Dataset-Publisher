...
export default async function SubmitPage({ params }: SubmitPageProps) {
  ...
  return (
    <div className="container mx-auto py-10 px-4">
      <div className="mb-6">
        <Link href={`/datasets/${params.id}`} className="text-primary hover:underline flex items-center">
          <svg
      ...
      {!canSubmit && !metadata && (
        <div className="bg-yellow-50 border border-yellow-200 rounded-md p-6 mb-6">
          ...
          <Link
            href={`/datasets/${params.id}/metadata`}
      ...
      {canSubmit && (
        <div className="bg-white p-6 rounded-lg shadow-sm border">
          ...
          <SubmitForm datasetId={params.id} versionId={latestVersion.id} />
        </div>
      )}
    </div>
  );
